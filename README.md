...
2.A - The first big difference is that `display: block;` has whitespace above and below it, and tolerates no HTML elements next to it, except when ordered otherwise. `display: inline;` is inside of the current block, allowing for content on either side by default.

- Another difference is that `display: block;` will always start on a new line, and takes up the full width of the page, from left to right. A block-level element can take up one line or multiple lines and has a line break before and after the element. `display: inline;` does not cause a line break. inline elements will also ignore the width and height properties.

3. What are the 4 areas of the box model?

3.A - The four areas of the box model are content, padding, border, and margin.

4. While using flexbox, what axis does the following property work on: `align-items: center`?

4.A - The `align-items: center` property works on the cross-axis.
